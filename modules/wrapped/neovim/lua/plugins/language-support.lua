return {
  {
    ...
    after = function(plugin)
      -- 1. Define global highlight groups
      -- vim.api.nvim_set_hl(0, "@CryptoProc", { link = "Keyword" })
      vim.api.nvim_set_hl(0, "@proc.cmd", { link = "Keyword" })
      vim.api.nvim_set_hl(0, "@assignment", { link = "Operator" })

      -- 2. Load custom queries from live nixconfig or nix store
      local function load_custom_query(lang, rel_path)
        local live_path = vim.fn.expand("~/nixconfig/modules/wrapped/neovim/" .. rel_path)
        local target_path = (live_path and vim.uv.fs_stat(live_path)) and live_path
          or (nixInfo(nil, "settings", "config_directory") and (nixInfo(nil, "settings", "config_directory") .. "/" .. rel_path))
        if target_path then
      ...
      load_custom_query("latex", "queries/latex/highlights.scm")
